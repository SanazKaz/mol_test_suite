...
def plot_pass_fail_rates(input_data: list, output_file: str):
    ...
    failed_color = "white"  # Same as passed, but with strike-through hatch
    ...
    ax.bar(
        ...
        counts_df["Invalid (%)"],
        width=bar_width,
        bottom=counts_df["Valid (%)"],
        color=failed_color,
        hatch="///",       # Strike-through style hatch
        edgecolor=passed_color,
        linewidth=0.0,
    )

    # Add percentage labels for Valid
    ...
    if len(ordered_methods) > 1:
        ...
        for m1, m2 in pairs_to_compare:
            contingency_table = [
                [counts_df.loc[m1, PASS_LABEL], counts_df.loc[m1, FAIL_LABEL]],
    ...
    # --- 5. Finalise and Save the Plot ---
    ax.set_ylabel("Percentage of predictions")
    ax.set_xlabel(None)
    ax.set_ylim(105)
    ax.set_yticks([0, 20, 40, 60, 80, 100])  # keep 100 as top tick
    ax.set_xticks(list(x))

    new_xticklabels = [
        f"{method}\n(N={counts_df.loc[method, 'Total']:,})"
    ...
    # Legend – only keep PB-Valid
    ax.legend([ax.patches[0]], [PASS_LABEL], frameon=False, loc="upper right", title="Result")
